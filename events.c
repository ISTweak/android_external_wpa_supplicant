...
/* [WLAN][SHARP] 2010.12.27 scan interval adjustment as STA can connect to AP immediatly after exit from panel-save inside WLAN cell mod Start */
#if 1
	static int periodic_scan_num = 1;
	#define SH_MAX_SCAN_INTERVAL 60 /* sec */
#ifndef MIN
	#define MIN(x,y) ((x)<(y)?(x):(y))
#endif
#endif
/* [WLAN][SHARP] 2010.12.27 scan interval adjustment as STA can connect to AP immediatly after exit from panel-save inside WLAN cell mod End */
...
/* [WLAN][SHARP] 2010.12.27 extend interval mod Start */
		periodic_scan_num = 1;
/* [WLAN][SHARP] 2010.12.27 extend interval mod End */
...
/* [WLAN][SHARP] 2010.12.27 extend interval mod Start */
#if 1
		timeout = MIN( SH_MAX_SCAN_INTERVAL, periodic_scan_num * timeout );
		if ( timeout < SH_MAX_SCAN_INTERVAL ){
		    periodic_scan_num = periodic_scan_num * 2; /* 1, 2, 4, 8, 16, .. */
		}
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add Start */
	wpa_printf(MSG_DEBUG, "wpa_supplicant_event_disassoc");

	if (wpa_s->current_ssid != NULL) {
		wpa_s->prev_ssid = wpa_s->current_ssid;
		wpa_printf(MSG_DEBUG, "wpa_s->prev_ssid = wpa_s->current_ssid setted");
	}
