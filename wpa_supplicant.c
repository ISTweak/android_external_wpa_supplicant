...
/* [WLAN][SHARP] 2011.04.14 Clear last associated SSID at Authentication timeout Add Start */
	wpa_s->current_ssid = NULL;
/* [WLAN][SHARP] 2011.04.14 Clear last associated SSID at Authentication timeout Add End */
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add Start */
	static Boolean second_scan_after_disconnected = FALSE;
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add End */
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected del Start */
//	ssid = wpa_s->conf->ssid;
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected del End */

/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add Start */
#if 1
	if(wpa_s->prev_ssid){ /* scan last associated AP */
		ssid = wpa_s->conf->ssid;
		while (ssid) {
		...
		second_scan_after_disconnected = TRUE;
	} else if(second_scan_after_disconnected){ /* scan boradcast */
		ssid = NULL;
		second_scan_after_disconnected = FALSE;
	} else{ /* scan according to wpa_supplicant.conf */
#endif
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add End */
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected mod Start */
#if 0
		if (!ssid->disabled &&
#else
		if (
#endif
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected mod End */
			      ...
			      struct wpa_ssid *ssid)
{
...
#if 0 /* [WLAN][SHARP] 2010/08/07 : to allow SharedKey authentication when retry */
	int assoc_failed = 0;
#else
	static int assoc_failed = 0;
#endif
	...
	if (ssid->auth_alg) {
		algs = 0;
		if ( (ssid->auth_alg & WPA_AUTH_ALG_OPEN) && (ssid->auth_alg & WPA_AUTH_ALG_SHARED) ) {
		    if (!(assoc_failed & (int)1) ){
			algs = AUTH_ALG_OPEN_SYSTEM;
		    } else{		   
			algs = AUTH_ALG_SHARED_KEY;
		    }
		} else if (ssid->auth_alg & WPA_AUTH_ALG_OPEN) {
		    algs = AUTH_ALG_OPEN_SYSTEM;
		} else if (ssid->auth_alg & WPA_AUTH_ALG_SHARED) {
		    algs = AUTH_ALG_SHARED_KEY;
		} else if (ssid->auth_alg & WPA_AUTH_ALG_LEAP) {
		    algs = AUTH_ALG_LEAP;
		}
...
/* [WLAN][SHARP]20101028 Modify for Hotspot. start */
#if 1
		   ...
		   (WPA_KEY_MGMT_PSK | WPA_KEY_MGMT_IEEE8021X |
		    WPA_KEY_MGMT_WPA_NONE)) {
#endif
/* [WLAN][SHARP]20101028 Modify for Hotspot. end */
		wpa_ie_len = sizeof(wpa_ie);
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add Start */
	wpa_printf(MSG_DEBUG, "wpa_supplicant_disassociate");
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add End */
...
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add Start */
	wpa_printf(MSG_DEBUG, "wpa_supplicant_deauthenticate");
/* [WLAN][SHARP] 2011.03.08 scan last associated SSID first after disconnected add End */
