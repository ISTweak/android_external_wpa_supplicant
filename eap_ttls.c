...
/* [WLAN][SHARP] 2011.06.07 ca verification Start */
#if 1   /* sharp modification:check ca cert */
	if (config) {
		if (config->ca_cert == NULL) {
			wpa_printf(MSG_INFO, "EAP-TTLS: Failed to get CACERT");
			eap_ttls_deinit(sm, data);
			return NULL;
		}
	}
#endif  /* sharp modification:check ca cert */
/* [WLAN][SHARP] 2011.06.07 ca verification End */
	return data;
}
